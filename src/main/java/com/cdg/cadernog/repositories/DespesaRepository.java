...
public interface DespesaRepository extends JpaRepository<DespesaModel, Long> {
    ...
    @Query("SELECT SUM(d.value) FROM DespesaModel d WHERE year(d.created_at) = ?1 AND month(d.created_at) = ?2")
    float monthlyTotal(int year, int month);

    @Query("SELECT SUM(d.value) FROM DespesaModel d WHERE year(d.created_at) = ?1")
    float annualTotal(int year);

    @Query("SELECT d FROM DespesaModel d WHERE year(d.created_at) = ?1 AND month(d.created_at) = ?2")
    List<DespesaModel> findAllByPeriod(int year, int month);
